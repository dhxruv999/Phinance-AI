...
def extract_features_from_url(url):
    ...
    domain_special = set(domain) - set('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.-') if domain else set()
    ...
    having_repeated_digits_in_domain = has_repeated_digits(domain) if domain else 0
    ...
    if domain:
        ...
        if number_of_subdomains > 0:
            ...
            average_subdomain_length = sum(len(s) for s in subdomains) / len(subdomains) if subdomains else 0
            average_number_of_dots_in_subdomain = subdomain_text.count('.') / len(subdomains) if subdomains else 0
            average_number_of_hyphens_in_subdomain = subdomain_text.count('-') / len(subdomains) if subdomains else 0
            
            subdomain_special = set(subdomain_text) - set('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.-')
            ...
            having_repeated_digits_in_subdomain = has_repeated_digits(subdomain_text)
        else:
...
def get_domain_reputation_score(domain):
    ...
    if hyphen_count > 2:
        ...
        suspicious_patterns = ['secure', 'verify', 'update', 'login', 'account', 'confirm']
...
def predict_url(url, model, features, threshold=0.75):
    ...
    if reputation_score >= 0.75:  # High reputation (likely legitimate)
        ...
        if reputation_score >= 0.85:
            ...
            target_legitimate = 0.70 + (reputation_score - 0.85) * 1.0  # Maps 0.85->0.70, 1.0->0.85
            ...
            target_legitimate = 0.60 + (reputation_score - 0.75) * 1.0  # Maps 0.75->0.60, 0.85->0.70
...
def main():
    ...
    try:
        ...
        print(f"Model accuracy: {model_info['accuracy']:.4f} ({model_info['accuracy']*100:.2f}%)")
    ...
    try:
        prediction, probability, reputation_score = predict_url(url, model, features, threshold=0.75)
    except Exception as e:
